...
const Dashboard = () => {
	...
	// update the gauge data whenever the current time frame data changes
	useEffect(() => {
		const maxValues = {
	...
	}, [currentTimeFrameData, timeFrame]); // the graph will be fill according to this date
	...
	// expense distribution data for the chart
	...
	const displayedIncome =
		showAllIncome ? incomeListForDisplay : incomeListForDisplay.slice(0, 3);

	const displayedExpense =
		showAllExpense ? expenseListForDisplay : expenseListForDisplay.slice(0, 3);

	// to fetch from the server-side
					...
					additionalContent={
						<div className='mt-2 text-xs text-cyan-600 flex items-center gap-2'>
							...
							{typeof overviewMeta.savingsRate === "number" && (
								<span
									className={`px-2 py-1 rounded-full text-xs font-medium ${overviewMeta.savingsRate < 0 ? trendStyles.negative : trendStyles.positive}`}>
				...
				<div className={dashboardStyles.listContainer}>
					...
					<div className={dashboardStyles.transactionList}>
						...
						{expenseListForDisplay.length === 0 && (
							<div className={dashboardStyles.emptyState}>
								<div
									className={dashboardStyles.emptyIconContainer(
										"bg-orange-50",
									)}>
