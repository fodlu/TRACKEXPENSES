...
const ExpensePage = () => {
	...
	const filteredTransactions = useMemo(() => {
		...
		const now = new Date();
		const yearFromSelectedMonth =
			selectedMonth ? new Date(selectedMonth).getFullYear() : null;
		const monthFromSelectedMonth =
			selectedMonth ? new Date(selectedMonth).getMonth() : null;
		const yearFromTimeFrame =
			timeFrameRange?.start ?
				new Date(timeFrameRange.start).getFullYear()
			:	null;
		const monthFromTimeFrame =
			timeFrameRange?.start ? new Date(timeFrameRange.start).getMonth() : null;
	...
	const chartData = useMemo(() => {
		...
		filteredTransactions.forEach((transaction) => {
			...
			const point = data.find((d) =>
				timeFrame === "daily" ? d.hour === transDate.getHours()
				: timeFrame === "yearly" ? d.date.getMonth() === transDate.getMonth()
				: d.date.getDate() === transDate.getDate() &&
					d.date.getMonth() === transDate.getMonth(),
			);
			if (point) {
				const amount = Number(transaction.amount);
				if (Number.isFinite(amount)) point.expense += amount;
			}
		});
	...
	const handleEditTransaction = async () => {
		...
		if (!editingId || !description || !Number.isFinite(amount) || amount <= 0)
			return;
		try {
			const payload = {
				description,
